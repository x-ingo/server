...
class StreamsAudio:
    ...
    async def get_queue_item_stream_with_smartfade(
        ...
        if not crossfade_allowed:
            ...
            try:
                ...
                # the mix consumed the full fade-out tail but only credited the
                # (possibly trimmed) fadeout share to bytes_written - track the
                # difference so the duration bookkeeping below stays in media-time
    ...
    async def get_queue_flow_stream(
        ...
        while True:
            ...
            # the held-back crossfade tail is excluded from bytes_written but is
            # still this track's media-time - without it the reported duration
            # shrinks by up to the crossfade length and can even drop below the
            # play position after a seek near the end of the track (#5494)
